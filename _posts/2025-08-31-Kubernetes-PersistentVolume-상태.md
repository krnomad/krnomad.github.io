...
2. **PV 객체 삭제**: 해당 PV를 `kubectl delete pv <pv-name>` 명령어로 삭제합니다. 이 과정에서 **실제 데이터는 삭제되지 않습니다.** 단지 Kubernetes 클러스터 내의 PV 객체만 제거됩니다.
3. **PV 객체 재생성**: 삭제했던 PV의 YAML 파일을 이용해 PV를 다시 생성합니다.
