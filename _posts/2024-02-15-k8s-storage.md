...
description: "OpenEBS 기반 스토리지를 붙이고 PVC, IOPS 측정, 기본 StorageClass 변경까지 한 번에 점검하는 메모입니다."
image:
  path: /assets/img/covers/k8s-storage.svg
  alt: OpenEBS storage and PVC flow cover
---
...
Default storage class는 `kubectl get sc` 명령을 통해 확인할 수 있는데, name앞에 `(default)`라는 명칭에 붙게 된다. 
...
Default storage class를 다음과 같이 지정할 수 있다.
