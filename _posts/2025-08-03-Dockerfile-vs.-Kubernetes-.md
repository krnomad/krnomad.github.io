...
- **실행 방식:** 셸(`/bin/sh` 등)을 거치지 않고 **프로그램을 컨테이너의 PID 1으로 직접 실행**합니다.
- **핵심 이점:**
- **주의:** 셸의 특수 기능(환경 변수 자동 확장, 파이프 `|`, 리다이렉션 `>` 등)은 직접 사용할 수 없습니다. (필요하면 `["/bin/sh", "-c", "your command here"]` 형태로 명시적으로 셸을 호출해야 함.)
- **기억:** **`["직접", "실행", "하라"]`** **(PID 1, 우아한 종료)**

### 2. `shell form` (셸 폼)

- **작성 방식:** `executable param1 param2`
- **실행 방식:** Docker가 이 명령어를 `/bin/sh -c "명령어"` 형태로 **셸을 통해 실행**합니다.
- **핵심 이점:**
- **주의:**
- **기억:** **`"셸을 거쳐서 실행!"`** **(PID 1 문제, 간단한 스크립트용)**
...
-  **Kubernetes Pod 정의에서는** **`command`****와** **`args`** **필드를 가급적 사용하지 않는 것이 좋습니다.**YAML
